...
export async function addCopy(card,extra={}){
  const row={id:newId("copy"),game:card.game,cardId:card.cardId,printingId:card.printingId,name:card.name,setName:card.setName||"",setCode:card.setCode||"",collectionNumber:card.collectionNumber||"",image:card.image||"",condition:extra.condition||"NM",language:extra.language||card.catalogLanguage||card.language||"it",variant:extra.variant||card.variant||"",pricePaid:extra.pricePaid==null?null:extra.pricePaid,notes:extra.notes||"",createdAt:new Date().toISOString()};
